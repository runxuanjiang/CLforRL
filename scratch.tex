...
\begin{algorithmic}[1]
    \Require MDP \(M = (\mathcal{S}, \mathcal{A}, H, \mathbb{P}, R, \rho), \epsilon > 0, 0 \leq\delta \leq 1, R_{max} > 0\).
    \Procedure{Curriculum\_R-max}{$M, \epsilon, \delta, R_{max}$}
        \State $L \gets \mathcal{S} \times \mathcal{A} \times [H]$
        \State $M_{1} \gets M_L$
        \State \(\hat{M} \gets M_L\)
        \State $\pi_{1} \gets$ optimal policy for \(\hat{M}\)
        \State $i \gets 1$
        \While{Running $\pi_{i}$ on $M$ explores a state in \(L\) with probability higher than \(\frac{\alpha}{H*R_{max}}\)}
...
\begin{proof}[Proof]
    ...
    % By the Chernoff bound and the fact that \(S\) is a binomial distribution with moment generating function \(M_S(t) = (q + pe^t)^n\) where \(p = \frac{\alpha}{SAH^2 R_{max}}\) and \(q = 1-p\) we have that
    % \[P(S \geq K_1) \leq \]
\end{proof}
